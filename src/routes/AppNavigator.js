  ...
  <Tab.Navigator>
    <Tab.Screen // Home screen
      ...
      options={{ //Options
        ...
        tabBarIcon: () => ( //Tab bar icon options
          <Fontisto name="home" size={30} style={styles.tabbarstyle} />
        ),
      }}
    />
    <Tab.Screen //Matches screen
      name="Matches"
      component={MatchesScreen}
      options={{
        tabBarIcon: () => (
          <MaterialCommunityIcons
            name="star-four-points" size={30} style={styles.tabbarstyle}
          />
        ),
      }}
    />
...
function SettingsNavigator() { // Settings navigator using stack instead of tab (Stack means the screens appear on top of eachother without a tab bar only back buttons)
  return(
    <Stack.Navigator>
      <Stack.Screen name="Settings page" component={SettingsScreen} options={{title: "Settings"}}/> 
      <Stack.Screen name="Edit Profile" component={ProfileScreen}/>
    </Stack.Navigator>
  );
}

export const AppNavigator = () => ( //Exports navigator to be imported in App.js
  <NavigationContainer>
    <HomeNavigator />
  </NavigationContainer>
);
...
const styles = StyleSheet.create({ //Style sheet
  tabbarstyle: { //Style which is applied to all tab navigator icons to style them easily
    top: 2,
  },
});
